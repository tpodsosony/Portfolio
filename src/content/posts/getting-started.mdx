...
Code snippets declared in `.mdx` files are powered by [Expressive Code](https://expressive-code.com). By default, Spectre uses a variation of GitHub's dark theme with custom background colors. 
...
```ts ins={7} del={1,6} title="astro.config.ts"
