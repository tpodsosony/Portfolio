...
### Modifying Expressive Code theme

Code snippets declared in `.mdx` files are powered by [Expressive Code](https://expressive-code.com). If you want to change the theme of the code snippets (syntax highlighting, background color) you can provide your desired theme in the `astro.config.ts` file as such:

```ts ins={6-8} del={5} title="astro.config.ts"
// ...
export default defineConfig({
  // ...
  integrations: [
...
```
You can find the list of available themes here: [Expressive Code themes](https://expressive-code.com/guides/themes/).